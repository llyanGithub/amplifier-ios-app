...
+ (HomeViewController*) getInstance
{
    ...
    __strong static HomeViewController* _sharedWexObject = nil;
...
//    [self.homeButton addTarget:self action:@selector(homeButtonClicked) forControlEvents:UIControlEventTouchDown];
    ...
    // 注册该页面可以执行点击切换
    [self.homeButton addTarget:self.revealViewController action:@selector(revealToggle:) forControlEvents:UIControlEventTouchUpInside];
...
- (void) connectedButtonClicked
{
    ...
    ConnectedView* connectedView = (ConnectedView*)self.revealViewController.presentingViewController;
    NSLog(@"%@", connectedView);
